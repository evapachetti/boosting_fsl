...
class MetaTemplate(nn.Module):
    ...
    def train_loop(self, epoch, train_loader, optimizer):
        ...
        for i, (x, _) in enumerate(train_loader):
            ...
            if self.change_way:
                self.n_way = x.size(0)
            optimizer.zero_grad()
            _, _, loss, _, auroc = self.set_forward_loss(x)
        ...
        return avg_loss / iter_num, auroc_mean
    ...
    def test_loop(self, test_loader):
        ...
        print('%d Val AUROC = %4.2f%% +- %4.2f%%' % (iter_num, auroc_mean, 1.96 * auroc_std / np.sqrt(iter_num) ))
