...
for epoch in {100,400}; do
    for bs in {128,256,512}; do
        for wd in {0.01,0.001,0.0001}; do
            ...
            PRETRAINED_MODEL_PATH=~/ssl_trainings/self_supervised_simclr/pretrained_models_additional/$METATRAIN_DATASET/$FOLDER
            PRETRAINED_MODEL_PATH="${PRETRAINED_MODEL_PATH//$'\r'/}"
            OUTPUT_MODEL_PATH=~/ssl_trainings/deepBDC/finetuned_models_additional/5-shot/$METATEST_DATASET/simclr/$FOLDER
            ...
            --method meta_deepbdc --output_path "$OUTPUT_MODEL_PATH" \
            --pretrain_path "$PRETRAINED_MODEL_PATH" --csv_path_train "$TRAIN_PATH" --csv_path_val "$VAL_PATH" \
            --csv_path_test "$TEST_PATH" --image_size 224 --learning_rate 0.01 --weight_decay 0.00001 \
            --epoch 100 --margin 0.5 --milestones 40 --n_shot 5 --n_query 10 --train_n_way 2 --val_n_way 2 \
            --num_classes 4 --train_n_episode 100 --val_n_episode 100 --reduce_dim 256 
            ...
            --pretrain_method SimCLR --pretrain_path "$PRETRAINED_MODEL_PATH" --model_path "$MODEL_PATH" \
            --output_path "$OUTPUT_MODEL_PATH" \
            --csv_path_train "$TRAIN_PATH" --csv_path_val "$VAL_PATH" --csv_path_test "$TEST_PATH" \
            --image_size 224 --n_shot 5 --n_query 10 --test_n_way 2 --reduce_dim 256 --test_n_episode 100 --test_task_nums 5
        done
    done
done
