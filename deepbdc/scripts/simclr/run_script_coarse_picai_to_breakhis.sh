...
for epoch in {100,400}; do
    for bs in {128,256,512}; do
        for wd in {0.01,0.001,0.0001}; do
            ...
            OUTPUT_MODEL_PATH=~/ssl_trainings/deepBDC/finetuned_models_additional/5-shot/$METATEST_DATASET/simclr_coarse_picai/$FOLDER
            ...
            --learning_rate 0.01 --weight_decay 0.00001 --epoch 100 --margin 0.5 --milestones 40 --n_shot 5 --n_query 10 \
            ...
            --pretrain_method SimCLR --model_path "$MODEL_PATH" --pretrain_path "$PRETRAINED_MODEL_PATH" --output_path "$OUTPUT_MODEL_PATH" \
            --csv_path_train "$TRAIN_PATH" --csv_path_val "$VAL_PATH" --csv_path_test "$TEST_PATH" \
            --image_size 128 --n_shot 5 --n_query 10 --test_n_way 2 --reduce_dim 256 --test_n_episode 100 --test_task_nums 5
        done
    done
done
