...
for epoch in {100,400}; do
    for bs in {128,256,512}; do
        for wd in {0.01,0.001,0.0001}; do
            ...
            ~/miniconda3/envs/evaenv/bin/python test.py --metatrain_dataset "$METATRAIN_DATASET" --metatest_dataset "$METATEST_DATASET" \
            --model "$MODEL" --method meta_deepbdc --pretrain_method SimCLR --pretrain_path "$PRETRAINED_MODEL_PATH" \
            --model_path "$MODEL_PATH" --output_path "$OUTPUT_MODEL_PATH" \
