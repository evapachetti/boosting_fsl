...
# for causality_setting in "mulcat"; do
...
# OUTPUT_FOLDER=${PRETRAINED_MODEL}_mulcat_lehmer_-100
...
# VAL_PATH="${VAL_PATH//$'\r'/}"
# TEST_PATH="${TEST_PATH//$'\r'/}"
# ~/miniconda3/envs/evaenv/bin/python test_binary_4_way.py --dataset picai --pretrain_method Imagenet --model ResNet18CA --method meta_deepbdc --model_path "$MODEL_PATH" --output_path "$OUTPUT_PATH"  --image_size 128 --n_shot 3 --n_query 10 --test_n_way 4 --reduce_dim 256 --test_n_episode 600 --test_task_nums 5 --csv_path_train "$TRAIN_PATH" --csv_path_val "$VAL_PATH" --csv_path_test "$TEST_PATH" --causality_aware --causality_method lehmer --causality_setting mulcat --lehmer_param -100 


## TRAINING AND TESTING BREAKHIS 4-WAY BASELINE (NO CAUSALITY) ##

OUTPUT_FOLDER=${PRETRAINED_MODEL}_baseline
...
#                 VAL_PATH="${VAL_PATH//$'\r'/}"
#                 TEST_PATH="${TEST_PATH//$'\r'/}"
#                 ~/miniconda3/envs/evaenv/bin/python meta_train.py --metatrain_dataset breakhis --metatest_dataset breakhis --model "$PRETRAINED_MODEL" --method meta_deepbdc --output_path "$OUTPUT_MODEL_PATH" --csv_path_train "$TRAIN_PATH" --csv_path_val "$VAL_PATH" --csv_path_test "$TEST_PATH" --image_size 224 --learning_rate 0.1 --weight_decay 0.01 --epoch 100 --margin 0.5 --milestones 40 --n_shot 1 --n_query 9 --train_n_way 4 --val_n_way 4  --train_n_episode 600 --val_n_episode 600 --reduce_dim 256 --causality_aware --causality_method "$causality_method" --causality_setting "$causality_setting" --lehmer_param "$lehmer_param" --ablation
#             done
#         else
#             OUTPUT_FOLDER=${PRETRAINED_MODEL}_${causality_setting}_${causality_method}
#             OUTPUT_MODEL_PATH=~/ssl_trainings/deepBDC/finetuned_models/1-shot/breakhis/causality/4_way/$OUTPUT_FOLDER
...
#             VAL_PATH="${VAL_PATH//$'\r'/}"
#             TEST_PATH="${TEST_PATH//$'\r'/}"
#             ~/miniconda3/envs/evaenv/bin/python meta_train.py --metatrain_dataset breakhis --metatest_dataset breakhis --model "$PRETRAINED_MODEL" --method meta_deepbdc --output_path "$OUTPUT_MODEL_PATH" --csv_path_train "$TRAIN_PATH" --csv_path_val "$VAL_PATH" --csv_path_test "$TEST_PATH" --image_size 224 --learning_rate 0.1 --weight_decay 0.01 --epoch 100 --margin 0.5 --milestones 40 --n_shot 1 --n_query 9 --train_n_way 4 --val_n_way 4  --train_n_episode 600 --val_n_episode 600 --reduce_dim 256 --causality_aware --causality_method "$causality_method" --causality_setting "$causality_setting" --ablation
...
#         if [[ "$causality_method" == "lehmer" ]]; then 
#             for lehmer_param in {-2,-1,0,1,100}; do
