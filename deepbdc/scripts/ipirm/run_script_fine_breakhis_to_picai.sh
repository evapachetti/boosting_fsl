...
for epoch in {100,400}; do
    for bs in {128,256,512}; do
        for wd in {0.01,0.001,0.0001}; do
            ...
            OUTPUT_MODEL_PATH=~/ssl_trainings/deepBDC/finetuned_models_additional/5-shot/$METATEST_DATASET/ipirm_fine_breakhis/$OUTPUT_FOLDER
            ...
            --metatest_dataset "$METATEST_DATASET" --model "$MODEL" --method meta_deepbdc --pretrain_method IPIRM \
            --model_path "$MODEL_PATH" --pretrain_path "$PRETRAINED_MODEL_PATH"  \
